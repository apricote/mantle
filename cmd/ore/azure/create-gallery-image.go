...
func runCreateGalleryImage(cmd *cobra.Command, args []string) error {
	...
	kr, err := api.GetStorageServiceKeysARM(storageAccount, resourceGrp)
	if err != nil {
	...
	for _, k := range *kr.Keys {
		if err := api.UploadBlob(storageAccount, *k.Value, vhd, container, blobName, true); err != nil {
			plog.Warningf("Uploading blob failed: %v", err)
			continue
		}
		break
	}
	blobUrl := api.UrlOfBlob(storageAccount, container, blobName).String()
