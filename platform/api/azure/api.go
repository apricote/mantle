...
func New(opts *Options) (*API, error) {
	...
	if opts.UseDefaultAuth {
		...
		credsInfo, err := internalAuth.ReadAzureCredentials(opts.AzureAuthLocation)
		if err != nil {
			return nil, fmt.Errorf("couldn't read Azure credentials JSON file: %w", err)
		}
		config = cloud.Configuration{
			ActiveDirectoryAuthorityHost: credsInfo.ActiveDirectoryEndpointURL,
		}
		authOpts := azidentity.ClientSecretCredentialOptions{
			ClientOptions: azcore.ClientOptions{
				Cloud: config,
			},
			// Not setting the AdditionallyAllowedTenants here, use
		...
		creds, err = azidentity.NewClientSecretCredential(credsInfo.TenantID, credsInfo.ClientID, credsInfo.ClientSecret, &authOpts)
		if err != nil {
			return nil, fmt.Errorf("failed to get client secret credentials: %w", err)
		}
		subID = credsInfo.SubscriptionID
	}
