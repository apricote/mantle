...
func (a *API) createPublicIP(resourceGroup string) (*armnetwork.PublicIPAddress, error) {
	...
	poller, err := a.ipClient.BeginCreateOrUpdate(context.TODO(), resourceGroup, name, armnetwork.PublicIPAddress{
		...
		Properties: &armnetwork.PublicIPAddressPropertiesFormat{
			DeleteOption: util.ToPtr(armnetwork.DeleteOptionsDelete),
		},
	}, nil)
	if err != nil {
		return nil, err
	...
	ip.Properties.DeleteOption = util.ToPtr(armnetwork.DeleteOptionsDelete)
	return &ip, nil
}
			...
			IPConfigurations: []*armnetwork.InterfaceIPConfiguration{
				{
					...
					Properties: &armnetwork.InterfaceIPConfigurationPropertiesFormat{
						PublicIPAddress:           ip,
						PrivateIPAllocationMethod: util.ToPtr(armnetwork.IPAllocationMethodDynamic),
						Subnet:                    subnet,
					},
				},
			},
			EnableAcceleratedNetworking: util.ToPtr(true),
		},
	}, nil)
	if err != nil {
		return nil, err
