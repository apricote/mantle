...
type Options struct {
	*platform.Options
	...
	// If true, try getting credentials from env var, k8s workload
	// identity, managed identity, azure cli or azure developer
	// cli. Otherwise get them from azure credentials JSON file.
	UseDefaultAuth bool

	// Opts for default auth:
	...
	// Opts for azure credentials JSON file auth:
