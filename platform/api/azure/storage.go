...
func DeleteBlob(client *service.Client, containerName, blobName string) error {
	...
	opts := blob.DeleteOptions{
		DeleteSnapshots: util.ToPtr(blob.DeleteSnapshotsOptionTypeInclude),
	}
...
func (a *API) CreateStorageAccount(resourceGroup string) (string, error) {
	...
	parameters := armstorage.AccountCreateParameters{
		SKU: &armstorage.SKU{
			Name: util.ToPtr(armstorage.SKUNameStandardLRS),
		},
		Kind:     util.ToPtr(armstorage.KindStorageV2),
...
func (a *API) GetStorageServiceKeys(account, resourceGroup string) ([]*armstorage.AccountKey, error) {
	opts := armstorage.AccountsClientListKeysOptions{
		Expand: util.ToPtr("kerb"),
	}
	res, err := a.accClient.ListKeys(context.TODO(), resourceGroup, account, &opts)
	if err != nil {
		return nil, err
	}
	return res.Keys, nil
